...
SELECT Employees.FirstName, Employees.LastName, Employees.PhoneNumber, EmployeePersonalInfo.Residence
FROM Employees
JOIN EmployeePersonalInfo ON Employees.EmployeeID = EmployeePersonalInfo.EmployeeID;

SELECT Employees.FirstName, Employees.LastName, EmployeePersonalInfo.BirthDate
FROM Employees
JOIN EmployeePersonalInfo ON Employees.EmployeeID = EmployeePersonalInfo.EmployeeID
WHERE EmployeePersonalInfo.MaritalStatus = 'Не замужем';

SELECT Employees.FirstName, Employees.LastName, EmployeePersonalInfo.BirthDate, Employees.PhoneNumber
FROM Employees
JOIN EmployeePersonalInfo ON Employees.EmployeeID = EmployeePersonalInfo.EmployeeID
JOIN EmployeeSalary ON Employees.EmployeeID = EmployeeSalary.EmployeeID
WHERE EmployeeSalary.Position = 'Менеджер';
